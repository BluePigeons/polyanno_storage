...
exports.voting = function(req, res) {
    ...
    var voteOn = polyanno.translation_model.findOne({'body.id':req.body.parent});
    voteOn.exec(function(err, translation) {
        ...
        var theLocation = polyanno.fieldMatching(translation.children, "id", req.body.children[0].id);
        ...
        var theChildDoc =  polyanno.fieldMatching(translation.children[thelocationIndex].fragments, "id", req.body.children[0].fragments[0].id);
        ...
        var fragmentChildByRank = function(therank) {
            return polyanno.fieldMatching(translation.children[thelocationIndex].fragments, "rank", therank);
        };
...
exports.findAll = function(req, res) {
    polyanno.translation_model.find(function(err, translations) {
        if (err) {res.send(err)}
...
var polyanno.jsonFieldEqual = function(docField, bodyDoc, bodyField) {
    if (polyanno.isUseless(bodyDoc[bodyField]) == false ) {    return bodyDoc[bodyField];    }
...
var votingInfoTexts = function(targetID, textArray, res) {
    ...
    var spanID = parts[1];
    polyanno.translation_model.findOne({'id': parentID}, function(err, textParent){
        if (err) { 
...
exports.addNew = function(req, res) {
    ...
    var translation = new polyanno.translation_model(); 

    var jsonFieldPush = function(bodyDoc, theField) {
        if ( polyanno.isUseless(bodyDoc[theField]) == false ) {
            bodyDoc[theField].forEach(function(subdoc){    translation[theField].addToSet(subdoc);    });
        };
    };
    var transURL = polyanno.translationURL.concat(translation.id);
    translation.body = bodySetting(translation.body, req.body.body);  
    translation.parent = polyanno.jsonFieldEqual(translation.parent, req.body, "parent");
    translation.translation = polyanno.jsonFieldEqual(translation.translation, req.body, "translation");

    jsonFieldPush(req.body, "metadata");

    var newChildrenArray = newChildrenChecking(translation.children, req.body.children);
    if ( ( polyanno.isUseless(translation.children) == false) && (polyanno.isUseless(newChildrenArray[0]) == false )
        && (newChildrenArray[0] != -1) ) {
        translation.children[newChildrenArray[0]].fragments.addToSet(newChildrenArray[1]);
    }
    else if ( ( polyanno.isUseless(translation.children) == false) && (polyanno.isUseless(newChildrenArray[0]) == false )
...
exports.getByID = function(req, res) {
    polyanno.translation_model.findById(req.params.translation_id, function(err, translation) {
        if (err) {res.send(err) }
...
exports.updateOne = function(req, res) {
    ...
    var updateDoc = polyanno.translation_model.findById(req.params.translation_id);
    updateDoc.exec(function(err, translation) {
        ...
        translation.parent = polyanno.jsonFieldEqual(translation.parent, req.body, "parent");
        translation.transcription = polyanno.jsonFieldEqual(translation.transcription, req.body, "transcription");
        jsonFieldPush(req.body, "metadata");
...
exports.deleteOne = function(req, res) {
    polyanno.translation_model.remove({
